export default async function handler(req, res) {
  ...
  const response = await fetch(
    `${supabaseUrl}/rest/v1/inNEWS?id=eq.${id}&select=Title,CoverImage`,
  ...
  const image = article.CoverImage || 'https://insaigon.app/og-image.png';
  const appUrl = `insaigon://insaigon.com/deeplink?articleId=${id}`;
...
<head>
  ...
  <meta property="og:description" content="Read on inSAIGON" />
